...
HOG_EC::HOG_EC(CommonTrie &trie) {
    l.resize(trie.t.size(), 1e9);
    r.resize(trie.t.size(), -1);

    int leaves_visited = 0;
    function<void(int)> dfs;
    dfs = [&leaves_visited, &trie, &dfs, this](int v){
        if(trie.t[v]->is_leaf()){
            l[v] = r[v] = leaves_visited;
            leaves_visited++;
        }
        for(auto &[a,child]:trie.t[v]->childs){
            dfs(child);
        }
        for(auto &[a,child]:trie.t[v]->childs){
            l[v] = min(l[v], l[child]);
